    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <span
            className="px-3 py-1 bg-yellow-100 text-yellow-700 rounded-full text-xs font-semibold">Warnings: 5</span>
        </div>
      </div>
            ...
            {inactiveCashiers.map((cashier) => (
              <div key={cashier.id}
                ...
                <div className="flex items-center gap-3">
                  <div
                    className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-gray-600 font-bold text-xs">
                ...
                <span
                  className="text-xs font-medium text-red-600 bg-red-50 px-2 py-1 rounded">{cashier.lastActive}</span>
              </div>
            ))}
          </div>
        </div>
              ...
              <div key={item.id} className="p-3 bg-gray-50 rounded-lg border border-gray-100">
                ...
                <div className="w-full bg-gray-200 rounded-full h-1.5">
                  <div className="bg-orange-500 h-1.5 rounded-full"
                       style={{width: `${(item.stock / item.threshold) * 100}%`}}></div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-3">
            {noSaleBranches.map((branch) => (
              <div key={branch.id}
                ...
                <span
                  className={`text-xs font-medium px-2 py-1 rounded ${branch.status === 'Closed' ? 'bg-gray-200 text-gray-600' : 'bg-yellow-100 text-yellow-700'}`}>
                  {branch.status}
                </span>
              </div>
            ))}
...
// Icons
const UserX = (props) => <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
...
const PackageAlert = (props) => <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
...
const StoreOff = (props) => <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                 fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"
                                 strokeLinejoin="round" {...props}>
