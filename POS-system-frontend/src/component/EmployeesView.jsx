...
const EmployeesView = () => {
  ...
  const [employees, setEmployees] = useState([
    { id: 1, name: "Rahul Sharma", role: "Cashier", email: "rahul.s@example.com", phone: "9876543210", status: "Active", joinDate: "2024-01-15" },
    { id: 2, name: "Priya Patel", role: "Manager", email: "priya.p@example.com", phone: "9876543211", status: "Active", joinDate: "2023-11-20" },
    { id: 3, name: "Amit Kumar", role: "Staff", email: "amit.k@example.com", phone: "9876543212", status: "On Leave", joinDate: "2024-03-10" },
    { id: 4, name: "Sneha Gupta", role: "Cashier", email: "sneha.g@example.com", phone: "9876543213", status: "Inactive", joinDate: "2024-02-01" },
  ]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <button onClick={() => { setEmployeeToEdit(null); setIsModalOpen(true); }} className="flex items-center justify-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-sm font-medium w-full sm:w-auto">
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="text-xs text-gray-500 dark:text-gray-400 border-b border-gray-100 dark:border-gray-700 uppercase tracking-wider">
              ...
              <th className="pb-3 font-medium px-4">Email Login Access</th>
              <th className="pb-3 font-medium px-4">Assigned Since</th>
          ...
          <tbody className="text-sm">
            {filteredEmployees.map((emp, i) => (
              <tr key={i} className="border-b border-gray-50 dark:border-gray-700/50 last:border-0 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                ...
                <td className="py-3 px-4 text-gray-500 dark:text-gray-400">{emp.email}</td>
                <td className="py-3 px-4 text-gray-500 dark:text-gray-400">{emp.joinDate}</td>
                <td className="py-3 px-4 text-right flex justify-end gap-2">
                  <button onClick={() => handleEditClick(emp)} className="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 transition-colors">
                    <Edit className="w-4 h-4" />
                  </button>
                  <button onClick={() => handleDeleteClick(emp.id)} className="text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300 transition-colors">
