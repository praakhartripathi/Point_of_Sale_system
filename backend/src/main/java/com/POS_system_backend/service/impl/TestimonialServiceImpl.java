...
public class TestimonialServiceImpl implements TestimonialService {
    ...
    public Testimonial updateTestimonial(Long id, Testimonial testimonialDetails) {
        ...
        if (testimonialDetails.getCustomerName() != null)
            testimonial.setCustomerName(testimonialDetails.getCustomerName());
        if (testimonialDetails.getBusinessName() != null)
            testimonial.setBusinessName(testimonialDetails.getBusinessName());
        if (testimonialDetails.getBusinessType() != null)
            testimonial.setBusinessType(testimonialDetails.getBusinessType());
        if (testimonialDetails.getRating() > 0) testimonial.setRating(testimonialDetails.getRating());
        if (testimonialDetails.getTestimonialText() != null)
            testimonial.setTestimonialText(testimonialDetails.getTestimonialText());
